 ...
 *
 */
public class ImportDataJN

{
	private Logger log = Logger.getLogger(ImportDataJN.class);
...
//				encode = getFileEncode(dir + fileName);
				...
				if (xmlDocument != null) {
					...
					int m = 0;
					m = xmlDocument.getChildLength("workgrid2.request.data");
					log.info(m);
					for (int j = 0; j < m; j++) {
						String keyvalue = "";
						...
						if (m == 1) {
							...
							if (must.equals("") == false) {
								for (int x = 0; x < must.split(",").length; x++) {
									keyvalue = xmlDocument.get("workgrid2.request.data."
											+ must.split(",")[x]);
							...
							if (flag) {
								...
								for (String key : keylist) {
									...
									if ("hos".equals(key)) {
										if (!"".equals(keyvalue)) {
											// 连云港版本
										   if( BiosanUtils.getFileNode("/dbcp.properties", "department_compare").equals("1"))
										   {
											String hoscode_old = keyvalue.split("\\|")[0];
											String select_newhos = "select HOSCODE_NEW || '|' || HOSNAME_NEW from ground_tb_department_compare where HOSCODE_OLD = '" + hoscode_old + "'";
							            	Object newhosObj = BiosanUtils.getSingle(select_newhos);
							            	if (newhosObj == null)
							            		continue;
						            		keyvalue = newhosObj.toString();
...
//								employeeguid = importbean.getHos().split("\\|")[0];
					...
					if (datalist.size() != 0) {
						...
						// 空文件或缺节点文件，重命名
						fileUtil.renameFile(dir + fileName, dir + fileName + "_error");
					}
				} else
					continue;
			} catch (Exception e) {
	 ...
	 * @param datalist
	 */
	private void insertData_(String renamefile, ArrayList<ImportBean> datalist) {
		log.info("写数据库！");
		// 判断存在
		...
		String insert_sql = "insert into ground_tb_importtemp(ID,SAMPLESTRING,BARCODESTRING,PATIENTNAME,BIRTHDAY,"
				...
				+ " PARAMER6,istosix,antibiotic,apgar,IMPORTDATE,PREGCARENUM,doc,bron_type,OPERATEDATE)" 
				+ " values(GROUND_SEQ_IMPORTTEMP.nextval,?,?,?,?,"
				+ " ?,?,?,?,?,"
				+ " ?,?,?,?,?,"
				+ " ?,?,?,?,?,"
				+ " ?,?,?,?,?,"
		...
		String update_sql = "update ground_tb_importtemp set SAMPLESTRING=?,BIRTHDAY=?,MOBILE=?,TELEPHONE=?,SEX=?,"
				...
				+ " antibiotic=?,apgar=?,PREGCARENUM=?,doc=?,bron_type=?,OPERATEDATE=? where patientname=? and BARCODESTRING=?";
		...
		try {
			...
			for (ImportBean importbean : datalist) {
				try {
					Object[] select_sql_params = { importbean.getPatientname(), importbean.getBarcode() };
					Object isnull = BiosanUtils.getSingle(select_sql, select_sql_params);
					// 处理实验项目
					String exp = importbean.getExperimentitem();
					exp = "labitem_" + exp;
					exp = exp.replaceAll(",", ",labitem_");
					if (isnull == null) {
						Object[] insert_sql_params = { importbean.getSamplestring(), importbean.getBarcode(), importbean.getPatientname(), importbean.getBirthday(), 
								importbean.getMobile(), importbean.getTelephone(), importbean.getSex(), importbean.getPreg_realcycle(), importbean.getChildname(), 
								importbean.getAddress(), importbean.getWeight(), importbean.getAdmissionnum(), exp, importbean.getCollectedate(), 
								importbean.getArea(), importbean.getHos(), importbean.getHousehold(), importbean.getHousetype(), importbean.getRemark(), 
								importbean.getParamer1(), importbean.getParamer2(), importbean.getParamer3(), importbean.getParamer4(), importbean.getParamer5(), 
								importbean.getParamer6(), importbean.getIstosix(), importbean.getAntibiotic(), importbean.getApgar(), importbean.getReserved1(), 
								importbean.getCollector(), importbean.getBron_type(), importbean.getOperatedate() };
						int insert_success = BiosanUtils.executeUpdate(insert_sql, insert_sql_params);
					} else {
						Object[] update_sql_params = { importbean.getSamplestring(), importbean.getBirthday(), 
								importbean.getMobile(), importbean.getTelephone(), importbean.getSex(), importbean.getPreg_realcycle(), importbean.getChildname(), 
								importbean.getAddress(), importbean.getWeight(), importbean.getAdmissionnum(), exp, importbean.getCollectedate(), 
								importbean.getArea(), importbean.getHos(), importbean.getHousehold(), importbean.getHousetype(), importbean.getRemark(), 
								importbean.getParamer1(), importbean.getParamer2(), importbean.getParamer3(), importbean.getParamer4(), importbean.getParamer5(), 
								importbean.getParamer6(), importbean.getIstosix(), importbean.getAntibiotic(), importbean.getApgar(), importbean.getReserved1(), 
								importbean.getCollector(), importbean.getBron_type(), importbean.getOperatedate(), 
								importbean.getPatientname(), importbean.getBarcode()};
						int update_success = BiosanUtils.executeUpdate(update_sql, update_sql_params);
					}
				} catch (Exception e) {
					e.printStackTrace();
					log.info("处理失败！samplestring" + importbean.getSamplestring());
				}
				j++;
			}
			log.info("提交！");
		...
		try {
			...
			for (ImportBean importbean : datalist) {
				...
				if (BiosanUtils.getFileNode("/dbcp.properties", "encrypt") != null) {
					...
					log.info("encrypt value is..." + encrypt);
					if ("".equals(encrypt) == false) {
					    
						encrypt = "1";
					}
				}
				log.info("final encrypt is..." + encrypt);
	...
	//				statement.setString(16, importbean.getTelephone());// 电话
				...
				if (departmentid.equals("")) {
					...
					log.info("obj area_ul====" + importbean.getArea());
			...
			log.info(sql_sample_batch_mapping);
			log.info("提交！");
			connection.commit();
			log.info("写库完成！");
